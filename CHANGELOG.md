...
### Changed
- **CLI default flipped to validate-only (breaking).** Running `latex2ufdissertation <input>` now validates the LaTeX source and stops — it no longer compiles. Compilation and the PDF layer (UF-F2/F3/S1/S5) are opt-in via the new `--compile` flag. This makes the tool a validator by default and lets students without a TeX install get full source-layer validation. A bare `.pdf` input still runs the PDF layer directly; `--compile` is a no-op there (warns).
...
### Removed
- **`--dry-run` flag removed (breaking).** It meant "validate, skip compile" — now the default, so the flag is obsolete. The framing note and demo hint that referenced it were updated to point at `--compile`.
